...
class PythonParser:
    ...
    def handle_string(self, value: str, prev_value: str, prefixed: bool, spacer: str) -> bool:
        ...
        if is_multi_line(value):
            ...
            if prefixed:
                self.html += "<span class=\"python-str\">{}</span>".format(value)
            else:
                self.html += "<span class=\"python-str\">{x}{y}</span>".format(x=spacer, y=value)
            return False
    ...
    def handle_comment(self, start: int, value: str) -> None:
        ...
        self.add_line_number()
        spacer = "&nbsp;" * start
        self.html += "<span class=\"python-comment\">{x}{y}</span>".format(x=spacer, y=value)
        self.html += "</code>"
        self.line_number += 1
